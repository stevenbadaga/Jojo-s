...
const PaymentConfirmationModal = ({ isOpen, onClose, orderData }) => {
  ...
  const orderId = orderData?.id || orderData?.orderNumber || 'MM-' + Math.floor(100000 + Math.random() * 900000)
  ...
  const handleSubmitPayment = (e) => {
    ...
    setTimeout(() => {
      ...
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('💳 MarketMet Payment Submitted', {
  ...
  const handleWhatsAppVerify = () => {
    const message = `*MARKETMET PAYMENT CONFIRMATION*%0A%0A` +
      `📦 *Order ID:* #${orderId}%0A` +
              ...
              {(paymentMethod === 'momo' || paymentMethod === 'airtel') && (
                <div className="bg-gradient-to-br from-[#002524] to-[#003834] border border-emerald-700 text-white p-5 rounded-2xl space-y-3.5 shadow-lg">
                  ...
                  <div className="bg-black/40 border border-emerald-600/40 p-3.5 rounded-xl flex items-center justify-between font-mono font-black text-base text-emerald-400">
                    ...
                    <span className="text-xs text-white font-sans font-bold bg-[#108910] px-2.5 py-0.5 rounded-md">MarketMet</span>
                  </div>
